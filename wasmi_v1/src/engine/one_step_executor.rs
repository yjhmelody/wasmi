...
pub fn execute_one_step<'engine>(
    ...
    Executor::new(ctx, frame, cache, inst, value_stack).execute()
}

/// An execution context for executing a `accel` one step instruction used for one step prove.
#[derive(Debug)]
struct Executor<'engine, 'func, Ctx> {
    ...
    /// The one instruction used for prove.
    inst: Instruction,
}
    ...
    Ctx: AsContextMut,
{
    ...
    /// Executes the function frame only one instruction.
